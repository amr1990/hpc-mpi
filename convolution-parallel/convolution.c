...
//    int headstored=0, imagestored=0, stored;
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    int imagesize, partitions, partsize, chunksize, halo, halosize, nparts, rank, nprocs;
    long position=0;
    ...
    MPI_Datatype imageStruct;
  	int blocks[12];
  	MPI_Aint indices[12];
    MPI_Datatype MPItypes[12];
    ...
    blocks[0] = 1;
  	...
  	MPI_Get_address( &data.data3, &indices[3] );
  	MPI_Get_address( &data.data3, &indices[4] );
  	MPI_Get_address( &data.data3, &indices[5] );
  	MPI_Get_address( &data.data3, &indices[6] );
  	MPI_Get_address( &data.data3, &indices[7] );
  	MPI_Get_address( &data.data3, &indices[8] );
  	MPI_Get_address( &data.data3, &indices[9] );
  	MPI_Get_address( &data.data3, &indices[10] );
  	MPI_Get_address( &data.data3, &indices[11] );
  	/* Make relative */
  	...
  	indices[5] = indices[5] - baseaddres;
  	indices[6] = indices[6] - baseaddres;
  	indices[7] = indices[7] - baseaddres;
  	indices[8] = indices[8] - baseaddres;
  	indices[9] = indices[9] - baseaddres;
  	indices[10] = indices[10] - baseaddres;
  	indices[11] = indices[11] - baseaddres;
  	MPI_Type_create_struct( 12, blocks, indices, MPItypes, &imageStruct );
    MPI_Type_commit( &imageStruct );
    ...
    if(rank == 0){
        ...
        SendData dataArray[nparts];
        SendData inDataArray[nparts];
        int x, y;
        int posR = 0, posG = 0, posB = 0;
        for(x = 0; x < nparts; x++){
          dataArray[x]->ancho = source->ancho;
          dataArray[x]->altura = source->altura/nparts;
          dataArray[x]->start = posR;
          for(y = 0; y < source->altura/nparts; y+){
            dataArray[X]->R[y] = source->R[posR];
            dataArray[x]->G[y] = source->G[posG];
            dataArray[x]->B[y] = sourxr->B[posB];
            ++posR;
            ++posG;
            ++posB;
          }
    ...
    //            if (source->R[i]!=output->R[i] || source->G[i]!=output->G[i] || source->B[i]!=output->B[i]) printf("At position i=%d %d!=%d,%d!=%d,%d!=%d\n",i,source->R[i],output->R[i], source->G[i],output->G[i],source->B[i],output->B[i]);
            ...
            int parts = 0;
            int status;
            SendData recvData, sendData;

            while(parts < nparts){
              MPI_Recv(recvData,12,imageStruct,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,&status);
              if(recvData->stat == -1){
                //TODO s'ha de controlar que s'envie, canviar el mySendArr per el struct que toqui
                // stat que s'ha d'enviar es 0
                // s'ha de ficar patiritions i halosize a sendData despres d'agafar el primer struct

                //if(hi ha dades que tractar)
                MPI_Send(mySendArr,12,imageStruct,status.MPI_SOURCE,0,MPI_COMM_WORLD);
                //else(enviar stat -1 per a que el proces fill pari)
                //MPI_Send(mySendArr,7,imageStruct,status.MPI_SOURCE,0,MPI_COMM_WORLD);

                ++parts;
              }else if(recvData->stat == -2){
                MPI_Recv(recvData,12,imageStruct,status.MPI_SOURCE,0,MPI_COMM_WORLD,&status);
                //TODO afegir el recvData a la array de structs de output
              }
            }

             //TODO recontruir la imatge convolucionada
      ...
      while(1){
        out->stat = -1;
        MPI_Send(out,12,imageStruct,0,0,MPI_COMM_WORLD);
        MPI_Recv(in,12,imageStruct,0,0,MPI_COMM_WORLD,&status);

        if(in->stat == -1){
          break;
        }else{
          convolve2D(in->R, out->R, in->ancho, (in->altura/in->partitions)+in->halosize, in->vkern, in->kernelX, in->kernelY);
          convolve2D(in->G, out->G, in->ancho, (in->altura/in->partitions)+in->halosize, in->vkern, in->kernelX, in->kernelY);
          convolve2D(in->B, out->B, in->ancho, (in->altura/in->partitions)+in->halosize, in->vkern, in->kernelX, in->kernelY);
